...
mincut::mincut(graph* _G, tree* _T)
{
	G = _G, T = _T;
	D = new lemma8ds(T);
	maxlog = ceil(log(T->n));
...
void mincut::dfs0(int node, int par, int ht)
{
...
void mincut::dfs1(int node, int par, int chain, double inc)
{
	inchain[node] = chain;
...
vector<pair<int, int>> mincut::pathbreakdown(int x, int y)
{
	if (H[x] > H[y]) swap(x, y);
	int origx = x, origy = y;
...
vector<pair<int, int>> mincut::ancestrybreakdown(int node, int anc)
{
...
double mincut::compute()
{
	...
	vector<vector<vector<edge*>>> events(int(st.size()), vector<vector<edge*>>(2));
	for (auto it : G->E)
	{
